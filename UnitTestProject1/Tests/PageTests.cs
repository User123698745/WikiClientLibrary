...
public class PageTests : WikiSiteTestsBase, IClassFixture<WikiSiteProvider>
{
    ...
    [InlineData(nameof(WikiaTestSiteAsync), "Project:sandbox", "Sandbox Wiki:Sandbox", BuiltInNamespaces.Project, 1518)]
    ...
    public async Task WikiPageReadTest2(string siteName, string fetchTitle, string expectedTitle, int expectedNs, int expectedId)
    {
        ...
        Utility.AssertNotNull(page.Content);
        Assert.True(page.ContentLength >= page.Content.Length,
            "ContentLength (in bytes) should be equal or greater than content characters.");
    ...
    public async Task WpEnPageGeoCoordinateTest()
    {
        ...
        await page.RefreshAsync(new WikiPageQueryProvider { Properties = { new GeoCoordinatesPropertyProvider() } });
        ShallowTrace(page);
    ...
    public async Task WikiPageWriteSectionTest1(string siteName, string pageTitle)
    {
        ...
        {
            Content = newSectionContent, Summary = SummaryPrefix + $"Append section ([[#{newSectionTitle}]]).", Minor = true, Bot = true,
        });
        ...
        {
            Content = newSectionContent, Summary = SummaryPrefix + $"Edit section ([[#{newSectionTitle}]]).", Minor = true, Bot = true,
        });
